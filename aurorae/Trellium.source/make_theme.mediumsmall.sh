...
displayname="Trellium $colorscheme (Medium Small)"
...
# remove 2px in button height
n=$(grep 'ButtonHeight=' "$name"rc | cut -d= -f2)
sed -i "s/ButtonHeight=$n/ButtonHeight=$((n - 2))/" "$name"rc
# remove 3px in button width
cat "$name"rc | grep 'ButtonWidth.*=' | while read line; do
	n=$(echo $line | cut -d= -f2)
	sed -i "s/$line/$(echo $line | cut -d= -f1)=$((n - 3))/" "$name"rc
done
